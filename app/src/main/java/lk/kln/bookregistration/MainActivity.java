...
public class MainActivity extends AppCompatActivity {
    ...
    public void addRecord(View view) {
        ...
        book = new Books(ISBN, name, author, publisher);
    ...
    public void viewData(View view) {
        ...
        for (int i = 1; i <= maxBookID; i++) {
            ...
            childReference.addValueEventListener(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    String isbnNo = dataSnapshot.child("isbno").getValue().toString();
                    String name = dataSnapshot.child("name").getValue().toString();
                    String author = dataSnapshot.child("author").getValue().toString();
                    String publisher = dataSnapshot.child("pulisher").getValue().toString();
                    System.out.println("ISBN No :" + isbnNo + "\t" + "Name :" + name + "\t" + "Author :"
                            + author + "\t" + "Publisher :" + publisher + "\t");
                }
